...
> 스크립트 작성부터 YouTube 업로드까지 — 사람 손 없이 매일 3편이 올라갑니다.
...
- **멀티 장르** — 미스터리, 다크 동물팩트, 심리학 3개 채널 콘셉트를 각기 다른 시간대에 자동 발행
...
| 장르 | 업로드 시간 (KST) | 워크플로 |
|------|-----------------|---------|
| 🔪 Mystery & Horror | 06:00 | `pipeline-mystery.yml` |
| 🐾 Dark Animal Facts | 12:00 | `pipeline-dark-animals.yml` |
| 🧠 Dark Psychology | 22:00 | `pipeline-psychology.yml` |

각 장르는 **독립된 GitHub Actions 워크플로**로 실행됩니다. PC를 켜 두거나 PM2가 필요 없습니다.
...
shorts-automation/
├── .github/workflows/
│   ├── pipeline-mystery.yml       ← UTC 21:00 / KST 06:00
│   ├── pipeline-dark-animals.yml  ← UTC 03:00 / KST 12:00
│   └── pipeline-psychology.yml   ← UTC 13:00 / KST 22:00
├── prompts/
│   ├── mystery.txt        ← 장르 바꾸려면 여기만 수정
│   ├── dark-animals.txt
│   └── psychology.txt
├── src/
...
> 4. 인증 흐름으로 refresh token 발급 (한 번만)
...
npm run mystery        # 미스터리 1편
npm run dark-animals   # 다크 동물팩트 1편
npm run psychology     # 다크 심리학 1편
```
