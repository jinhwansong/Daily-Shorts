...
- **멀티 장르(선택)** — 미스터리(Noctivault) 자동 + 심리학 채널은 토큰 준비 후 워크플로에서 cron 켜기
...
| 장르 | 자동 스케줄 | 워크플로 |
|------|------------|---------|
| 🔪 Mystery (Noctivault) | 매일 KST 06:00 (UTC 21:00) | `pipeline-mystery.yml` |
| 🧠 Dark Psychology | **비활성** — `YOUTUBE_REFRESH_TOKEN_PSYCHOLOGY` 넣은 뒤 `pipeline-psychology.yml`에서 cron 주석 해제 | `pipeline-psychology.yml` (지금은 **수동 실행만**) |

토큰 없이 심리학 워크플로 cron이 켜져 있으면 **API 비용만 나가고** 업로드는 실패할 수 있어, 기본은 **심리학 자동 실행을 꺼 둔 상태**입니다.
...
shorts-automation/
├── .github/workflows/
│   ├── pipeline-mystery.yml       ← cron: 미스터리 매일
│   └── pipeline-psychology.yml   ← 수동만 (토큰 후 cron 주석 해제)
├── prompts/
│   ├── mystery.txt
│   └── psychology.txt
├── src/
